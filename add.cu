...
__global__ void addKernel(int* c, const int* a, const int* b, int* result)
{
    int i = threadIdx.x;
    c[i] = a[i] + b[i];
    if (a[i] == 3) {
...
cudaError_t addWithCuda(int* c, const int* a, const int* b, unsigned int size, int* result)
{
    int* dev_a = 0;
    int* dev_b = 0;
    int* dev_c = 0;
    ...
    addKernel << <1, size >> > (dev_c, dev_a, dev_b, dev_result);
